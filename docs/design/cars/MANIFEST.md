...
| `car-16-sls-amg-cutout.png` | **the master the hero and tile are built from** — that render cut out onto transparency by the designer, 2299×958 RGBA, car only. One addition: the US amber side-marker his car has and the render lacked. **Repaired 23 Sep 2026:** the designer's cut had lost the rear quarter and bumper behind the rear wheel (render rows 704–948), which sit within a few levels of the black backdrop and cross it at the bumper's crease — the page showed a notch there. The outline was re-traced off the render's own row profiles and those 7,396 pixels restored from the render, unedited; nothing else changed (the designer's original is in git history). Rebuild with `python3 scripts/build-car-hero.py docs/design/cars/car-16-sls-amg-cutout.png mercedes-sls-amg --width 758 --hue 18-45 --soften 0.7` |
