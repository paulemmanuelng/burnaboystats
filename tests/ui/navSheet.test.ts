...
describe("mobile nav sheet", () => {
  ...
  it("lists 27 routes — 6 browse, 8 record books, 13 site", () => {
    ...
    // 17 Aug 2026: +/afrobeats, the genre board.
    ...
    expect(navGroups.map((g) => g.items.length)).toEqual([6, 8, 13]);
    expect(navRoutes).toHaveLength(27);
  });
