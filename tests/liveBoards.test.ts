...
import { LIVE_ARTISTS as LIVE_REGISTRY } from "../scripts/live-artists.mjs";
...
describe("the board's live charts", () => {
  ...
  it("is never empty, and never Burna Boy's data", () => {
    for (const b of LIVE_BOARDS) {
      ...
      // second number here, read the SAME declaration the build script uses —
      // scripts/live-artists.mjs `mayChartNowhere` — so the two cannot drift.
      // Everyone else still has to clear 25.
      const mayBeTiny = LIVE_REGISTRY[b.slug]?.mayChartNowhere === true;
      expect(b.placements, b.slug).toBeGreaterThan(mayBeTiny ? 0 : 25);
