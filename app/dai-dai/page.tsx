...
export default function DaiDaiPage() {
  const articleJsonLd = {
    ...
    // The share card by its real URL: generateImageMetadata puts the card's id
    // in the path, so the bare /dai-dai/opengraph-image 404s. The id is the
    // one the MusicEvent below cites (./ogId); the Spanish edition's Article
    // cites its own card the same way.
  ...
  const heroNumbers: { v: string; l: string }[] = [
    ...
    { v: `${daiDaiCertCount}`, l: "certifications — Diamond in France, 2× Platinum in Canada, 6× Platino in the US, Platinum in Spain, Slovakia, Portugal, Hungary, Austria, Greece & Sweden, Gold in Colombia, the Czech Republic, Italy, Poland, Belgium & Germany, and Silver in the UK" },
