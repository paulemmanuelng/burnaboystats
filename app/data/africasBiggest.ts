...
export const statBoxes: LeaderboardBox[] = [
    ...
    rows: [
      {
        ...
        entries: [
          ...
          /* live:streams-2026-wizkid */ { name: "Wizkid", value: "1.770B", tie: true },
          /* live:streams-2026-burna */ { name: "Burna Boy", value: "1.764B", tie: true },
  ...
  },
  {
    ...
    layout: "list",
    entries: [
      { name: "Burna Boy", sub: "🇳🇬 Nigeria", value: "15.13M" },
      { name: "Wizkid", sub: "🇳🇬 Nigeria", value: "14.85M" },
    ],
    note: "Burna Boy is past 15 million equivalent album sales, and his lead over Wizkid has stretched from about 30,000 to some 284,000 across the “Dai Dai” run. Read the scope with the figure: these two are the only artists from any African country on ChartMasters' 691-name board, and the only Afrobeats artists it has published an EAS total for at all.",
    source: "Total equivalent album sales (EAS), per ChartMasters' daily Best-Selling Artists of All-Time board: Burna Boy 15,132,000 (rank 534) to Wizkid's 14,848,000 (rank 538), read 10 September 2026 with the board's streams stamped 8 September. The 30 August reading was 15,011,000 (536) to 14,758,000 (539), so both men gained — Burna by 121,000, Wizkid by 90,000 — the gap widened from about 253,000 to some 284,000, and both moved up as the board grew from 688 names to 691. READ THE RANK FROM THE g# COLUMN, NOT THE # COLUMN: the leading # is a client-side row counter that resets to 1 under any search or filter, so a re-read that searches for a name and copies the first number will publish a rank of 1. g# is the real position, and sorting all 691 rows by Total EAS reproduces it exactly. IMPORTANT ON METHOD, still true: ChartMasters has not completed a CSPC study for either man — on both rows the studio-album, other-LP, physical-single and digital-single columns all read 0, and Total EAS equals the streaming figure to the digit. Checked against a negative control rather than assumed: neighbours who DO have studies render non-zero in the same read, e.g. Jung Kook at rank 532 with 3,075,000 studio and 2,780,000 digital singles. Board-wide, 231 of the 691 have all-zero sales columns and 460 have a completed study; these two are in the 231. So both totals are streaming-only estimates that would UNDERSTATE a real CSPC result, not full sales tallies. ChartMasters itself makes no claim about African artists anywhere on that page. THE NAME ANYONE WILL RAISE: Akon sits at rank 502 on 16,516,000 EAS, ahead of Burna Boy. ChartMasters tags his country as the United States, not Senegal, so the claim above is safe as written against this source — but he is Senegalese-American, and that is the challenge to expect. The two have traded this lead before, so it is worth re-reading rather than assuming."
  },
  {
