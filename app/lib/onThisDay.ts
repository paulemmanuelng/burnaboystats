...
/** The pill each kind wears — the update feed's words where it has one. */
export const KIND_LABEL: Record<OnThisDayKind, string> = {
  release: "Release",
  chart: "Charts",
  streaming: "Streaming",
...
/**
 * The ink each kind reads in: the update feed's palette (lib/updateInk.ts), so
 * a certification is the same gold and a chart peak the same cyan wherever the
 * site files one. Releases take --ember, the feed's "Firsts & Records" ink.
 */
export const KIND_INK: Record<OnThisDayKind, string> = {
  release: "var(--ember)",
  chart: "var(--cyan)",
...
export const onThisDayCounts = Object.fromEntries(
  (Object.keys(KIND_LABEL) as OnThisDayKind[]).map((k) => [k, onThisDayEvents.filter((e) => e.kind === k).length]),
